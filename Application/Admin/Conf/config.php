...
return array(
    ...
    /* 游戏文件原包相关配置 */
    'DOWNLOAD_UPLOAD_SHARD' => array(
        'mimes'    => '', //允许上传的文件MiMe类型
        'maxSize'  => 700*1024*1024, //上传的文件大小限制 (0-不做限制)
        'exts'     => 'zip,rar,apk,ios', //允许上传的文件后缀
        'autoSub'  => true, //自动子目录保存文件
        'subName'  => array('date', 'Y-m-d'), //子目录创建方式，[0]-函数名，[1]-参数，多个参数使用数组
        'rootPath' => './Uploads/SourcePack', //保存根路径
        'savePath' => '', //保存路径
        'saveExt'  => '', //文件保存后缀，空则使用原后缀
        'replace'  => false, //存在同名是否覆盖
        'hash'     => true, //是否生成hash编码
    ...
    'DOWNLOAD_UPLOAD_DRIVER'=>'Shard',
    'UPLOAD_Shard_CONFIG' => array(
    ...
    'EDITOR_UPLOAD' => array(
		'mimes'    => '', //允许上传的文件MiMe类型
		'maxSize'  => 2*1024*1024, //上传的文件大小限制 (0-不做限制)
		'exts'     => 'jpg,gif,png,jpeg,doc,docx,txt,xml,xlsx,xls', //允许上传的文件后缀
		'autoSub'  => true, //自动子目录保存文件
		'subName'  => array('date', 'Y-m-d'), //子目录创建方式，[0]-函数名，[1]-参数，多个参数使用数组
